...
description: "리서치-분류-분석 파이프라인으로 도출된 내 브랜드 '사단(士團)'을 기준으로 [클라이언트 생성 파일] 바탕 신규 가상 클라이언트를 생성하고, 분석_skill/분석_md를 발동시켜 '가상 클라이언트 기반 분석 결과 작성 양식.md' 양식으로 '메뉴, 콘텐츠, 기능'이 다채롭고 풍부하게 세분화된 사이트 분석 결과를 도출하여 [클라이언트 분석] 폴더에 저장하는 통합 자동화 스킬"
argument-hint: "[사단 브랜드 클라이언트 담당자 이름 (예: 한유진, 김도현, 이수진)]"
disable-model-invocation: true
---

# 클라이언트 자동화 통합 실행 스킬 (SKILL.md)

---

## 0. 스킬 개요 및 원칙
* **스킬 목적:** `클라이언트_자동화.md` 명세에 정의된 3단계 클라이언트 자동화 워크플로우를 순차적으로 실행한다.
* **기반 가이드:** [`클라이언트_자동화.md`](file:///C:/uiux_les/antygravity/%ED%81%B4%EB%9D%BC%EC%9D%B4%EC%96%B8%ED%8A%B8_%EC%9E%90%EB%8F%99%ED%99%94.md)
* **핵심 원칙:**
  1. **내 브랜드 = '사단(士團)' 고정 원칙:** 모든 가상 클라이언트는 리서치-분류-분석으로 도출된 내 브랜드인 **'사단 (士團)' 브랜드 프로젝트 관련 클라이언트/담당자**로 생산한다.
  2. **파일명 뒤 클라이언트 이름 부착:** 파일명 뒤에는 브랜드명이 아니라 **사단 관련 클라이언트 사람 이름**을 부착한다.
  3. **1 클라이언트 = 1 파일 원칙:** 클라이언트 하나당 반드시 하나의 독립된 전용 파일로 작성한다.
  4. **다채로운 메뉴, 콘텐츠, 기능 계층 구조 수립:** 단조로운 구성을 지양하고, **메뉴, 콘텐츠, 기능 요소를 각각 5~10개 이상 풍부하게 확장하여 세분화된 IA 트리 및 와이어프레임 구조**를 도출한다.
...
### [ Step 2 ] 분석_skill 발동 및 가상 클라이언트 기반 분석 작성
1. **발동 스킬:** `분석_skill.md`, `분석_md.md`
...
- [ ] **사단 브랜드 기준:** 내 브랜드인 '사단 (士團)' 브랜드 프로젝트 맥락에 맞게 작성되었는가?
