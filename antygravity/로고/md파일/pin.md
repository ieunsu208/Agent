...
- **수집 및 금지 조건**:
  ...
  - **동일/중복 이미지 절대 제외**: 기존 수집된 파일 및 동일 폴더 내 이미지와 동일한 중복 이미지(해시값 및 조형 일치)는 엄격히 제외
...
- **중복 방지 및 무결성 검증**: 수집된 이미지는 파일 해시(SHA-256) 검증을 통해 기존 수집본과 중복 시 즉시 건너뛰고 100% 새로운 고유 레퍼런스를 추출
